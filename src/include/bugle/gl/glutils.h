 ...
 * }
 * bugle_gl_begin_internal_render will return BUGLE_FALSE if one is inside begin/end.
...
BUGLE_EXPORT_PRE bugle_bool bugle_gl_begin_internal_render(void) BUGLE_EXPORT_POST;
BUGLE_EXPORT_PRE void bugle_gl_end_internal_render(const char *name, bugle_bool warn) BUGLE_EXPORT_POST;
 ...
 */
BUGLE_EXPORT_PRE void bugle_gl_filter_catches_drawing(filter *f, bugle_bool inactive, filter_callback callback) BUGLE_EXPORT_POST;
/* Like the above, but only immediate mode vertex commands */
BUGLE_EXPORT_PRE void bugle_gl_filter_catches_drawing_immediate(filter *f, bugle_bool inactive, filter_callback callback) BUGLE_EXPORT_POST;

/* Returns BUGLE_TRUE for glVertex*, and for glVertexAttrib* with attribute
 * 0. These are the calls that generate a vertex in immediate mode.
 * f must be canonicalised.
 */
BUGLE_EXPORT_PRE bugle_bool bugle_gl_call_is_immediate(function_call *call) BUGLE_EXPORT_POST;
