...
static void dump_wrapper(FILE *f, void *data)
{
    ...
    budgie_dump_any_type(w->type, w->out, w->length, f);
}

char *bugle_get_state_string(const glstate *state)
{
    GLdouble d[16];
    GLfloat f[16];
    GLint i[16];
    ...
    case STATE_MODE_TEXGEN:
        if (state->info->type == TYPE_8GLdouble)
        {
            CALL_glGetTexGendv(state->coord, state->info->pname, d);
            in_type = TYPE_8GLdouble;
        }
        else if (state->info->type == TYPE_7GLfloat)
        {
            CALL_glGetTexGenfv(state->coord, state->info->pname, f);
            in_type = TYPE_7GLfloat;
        }
        else
        {
            CALL_glGetTexGeniv(state->coord, state->info->pname, i);
            in_type = TYPE_5GLint;
        }
        break;
...
static void spawn_children_null(const glstate *self,
    ...
    for (info = table; info->name; info++)
    {
            ...
            || (info->extension != -1 && bugle_gl_has_extension(info->extension)))
        {
            ...
            *child = *self; // copies contextual info
            ...
            child->info = info;
            child->spawn_children = spawn_children_null;
